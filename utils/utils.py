...
def fix_json_error(data: str, return_str=True):
    print("UTILS: fix_json_error:", data)
    print("----")
    data = data.strip().strip('"').strip(",").strip("`")
    print("UTILS: fix_json_error STRIPPED:", data)
    print("----")
    try:
        json.loads(data)
        return data
    except json.decoder.JSONDecodeError:
        print("UTILS: json.decoder.JSONDecodeError")
        data = data.split("\n")
        # if the data[0] is 'json', then remove the first index
        print("UTILS: fix_json_error split before:", data)
        if data[0] == "json":
            data = data[1:]
        # remove any empty lines
        data = [line for line in data if line.strip() != ""]
        print("UTILS: fix_json_error split after:", data)
        print("----")
